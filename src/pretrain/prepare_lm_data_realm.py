        ...
        if i == len(document) - 1 or current_length >= target_seq_length:
            ...
            if current_chunk:  # 如果当前块不为空
                ...
                with torch.no_grad():
                    ...
                    attention_mask_a = [1] * len(tokens_a)
                    inputs = {"input_ids": tokens_a, "attention_mask": attention_mask_a, "start_positions": None, "end_positions": None}
                    ners = ner_model(inputs)

                # 创建masked LM的任务的数据 Creates the predictions for the masked LM objective
                tokens, masked_lm_positions, masked_lm_labels = create_masked_lm_predictions(
                    tokens, ners, max_ngram, masked_lm_prob, max_predictions_per_seq, vocab_words)
                instance = {
                    "tokens": tokens,
...
def create_masked_lm_predictions(tokens, ners, max_ngram, masked_lm_prob, max_predictions_per_seq, vocab_list):
    ...
    for (i, token) in enumerate(tokens):
        ...
        if token == "[SEP]": # YZ: only mask the first sentence
            break
        if ners[i] != "O": # YZ: only mask the importance word
            continue
